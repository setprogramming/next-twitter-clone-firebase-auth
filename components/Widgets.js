  ...
  return (
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
        ...
        <div className="text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%] sticky top-16">
          <h4 className="font-bold text-xl px-4 pb-3">
            Who to follow
          </h4>
          <AnimatePresence>
            {randomUsersResults.slice(0,randomUserNum).map((randomUser) => (
              ...
              transition={{ duration: 1 }}
            >
                <div key={randomUser.login.username} className="flex items-center px-4 py-2 cursor-pointer
                hover:bg-gray-200 transition duration-500 ease-out">
                  <img src={randomUser.picture.thumbnail} className="rounded-full" width="40"/>
          ...
          <button onClick={() => setRandomUserNum(randomUserNum + 5)} className="text-blue-300 pl-4 pb-3 hover:text-blue-400">
            Show more
          </button>
        </div>
